...
module.exports = {
        ...
        if (config.ADMIN_ACCESS.FREE_ROUTES.indexOf(path) > -1) {
                    ...
                    if (us.is_active && !us.is_deleted) {
                        if (config.ADMIN_ACCESS.NONAUTHORIZED_ONLY.indexOf(path) > -1)
                            return req.method != 'GET'
                                ? res.send({
                                    status: 0,
                                    message: '1Unauthorized access denied',
                                    data: null
                                })
                                : res.redirect('/');
                        else if (
                            ...
                            return req.method != 'GET'
                                ? res.send({
                                    status: 0,
                                    message: '2Unauthorized access denied',
                                    data: null
                                })
            ...
            if (config.ADMIN_ACCESS.NONAUTHORIZED_ONLY.indexOf(path) > -1)
                ...
                return req.method != 'GET'
                    ? res.send({
                        status: 0,
                        message: '3Unauthorized access denied',
                        data: null
                    })
    ...
    uploadFile: function (file, types) {
        return new Promise((resolve, reject) => {
            ...
            if (!mime_type) {
                //logger.info("Not a valid mime", newFileName);
                return resolve(false);
            }

            //logger.info("MIME FOUND", mime_type);
            ...
            file.mv('./public' + newFileName, function (err) {
                ...
                fs.readFile('./public' + newFileName, async function (err, data) {
                    if (err) throw err;
                    //logger.info("DATA", newFileName);
                    ...
                    s3bucket.createBucket(function () {
                        ...
                        s3bucket.upload(params, function (err, data) {
                            ...
                            //logger.info('success', JSON.stringify(data, undefined, 2));
